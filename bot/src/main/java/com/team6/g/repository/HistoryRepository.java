...
public interface HistoryRepository extends JpaRepository<History, Long> {
    ...
    @Query("SELECT new com.team6.g.model.HistoryStatistics(h.user, count(h)) FROM History h WHERE h.dateAdded BETWEEN ?1 AND ?2 GROUP BY h.user ORDER BY count(h) DESC")
    List<HistoryStatistics> findByDateBetweenAndGroup(Date startDate, Date endDate);

    @Query("SELECT new com.team6.g.model.HistoryStatistics(h.user, count(h)) FROM History h GROUP BY h.user ORDER BY count(h) DESC")
